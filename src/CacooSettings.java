...
public class CacooSettings {
    ...
    private static final String cacooAPI = "https://cacoo.com/api/v1/";
    private static final String apiKey = "apiKey=RV9uaLc9w69BTGCA2DXO";
    ...
    protected static HttpResponse cacooApi(HttpServletRequest req, HttpServletResponse resp, String url)
            throws Exception{
        // for HttpClient debugging
        System.setProperty("org.apache.commons.logging.Log","org.apache.commons.logging.impl.SimpleLog");
        System.setProperty("org.apache.commons.logging.simplelog.showdatetime","true");
    ...
    protected static JsonElement httpResponse2json(HttpResponse res){
        try{
            String s = EntityUtils.toString(res.getEntity());
            return new JsonParser().parse(s);
        }catch (Exception e) {
    ...
    /*
     * Create the URL to call the Cacoo API and retrieve the JSON object

    protected static JsonObject callCacooAPI(HttpServletRequest request, HttpServletResponse response,
                                             String apiCall, String format) {
        JsonObject root = null;
        try {
            String url = new String();
            if(format.equalsIgnoreCase("JSON")) url = cacooAPI + apiCall + JSON + "?" + apiKey;
            if(format.equalsIgnoreCase("XML")) url = cacooAPI + apiCall + XML + "?" + apiKey;
            HttpResponse res = cacooApi(request, response, url);
            JsonElement json = CacooSettings.httpResponse2json(res);
